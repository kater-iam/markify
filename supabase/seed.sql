...
SELECT
  ...
  ('00000000-0000-0000-0000-000000000000'::uuid + gs.i)::uuid AS id,
  'authenticated', 'authenticated',
  format('user%d@example.com', gs.i - 1),
  crypt('password123', gen_salt('bf')),
  current_timestamp, '', current_timestamp,
  '', current_timestamp, '', '',
  current_timestamp, current_timestamp,
...
SELECT
  ('00000000-0000-0000-0000-000000000000'::uuid + gs.i)::uuid AS id,
  ('00000000-0000-0000-0000-000000000000'::uuid + gs.i)::uuid AS user_id,
  format('user%03d', gs.i - 1) AS code,
  format('First%d', gs.i - 1) AS first_name,
  format('Last%d', gs.i - 1) AS last_name,
  'general', now(), now()
FROM generate_series(2, 31) AS gs(i);
...
SELECT
  ('00000000-0000-0000-0000-000000010000'::uuid + (gs.i - 1)) AS id,
  '00000000-0000-0000-0000-000000000001'::uuid AS profile_id,
  format('images/image_%03d.jpg', gs.i) AS file_path,
  now(), now()
...
SELECT
  ('00000000-0000-0000-0000-000000020000'::uuid + ((gs.i - 1) * 3 + log.j - 1)) AS log_id,
  -- Cycle through general users for logs
  ('00000000-0000-0000-0000-000000000000'::uuid + ((log.j - 1) % 30) + 2)::uuid AS profile_id,
  ('00000000-0000-0000-0000-000000010000'::uuid + (gs.i - 1)) AS image_id,
  format('192.168.0.%d', log.j + 10) AS client_ip,
  now(), now()
FROM generate_series(1, 50) AS gs(i)
